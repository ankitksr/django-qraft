### Asyncio Worker (Future Feature) — Design & Implementation Plan (TODO)
...
#### Task compatibility policy (must decide)
...
#### Task intake (mp queue → asyncio loop)
...
#### Result publishing (async → mp result_queue)
...
### Django integration considerations (Django 5.x async)
...
### Compatibility & safety gotchas (must document)
